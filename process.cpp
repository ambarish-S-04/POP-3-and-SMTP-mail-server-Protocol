...
//We create a function pointer. which will hold list of our pop3 commands.
...
//the sendresponse function, sends our commandreply, back to the client
	...
	for (int i = 0; i < listUser.size(); i++) 
	{
		if(listUser[i].email == User.userEmail)
		{
			if(listUser[i].lock == true)
				return false;
			listUser[i].lock = true; //lock the user mails
			User.mailLock = true;
...
//list of expected commands from the client.
...
int ProcessCMD(pop3User &User, char *clientMessage)
{
	...
	cout<<message<<endl;
	//loop through our function list and compare the command received from the client,
	//to the commands in the string called commands above.
	for(int i = 0; i < sizeof(funcArray); i++)
	{	
		//compare client command to expected command.
		if(strncmp(message.c_str(),commands[i].c_str(),4)== 0 && message.size() > 6)
		{
			//check that command is terminated with CRLF, and delete it (CRLF=> Carriage return, Line feed)
			...
			//on success call the corresponding function
